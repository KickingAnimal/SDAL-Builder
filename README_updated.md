...
The following notable changes have been made since the last revision of this README:

### 1. Build Process: OEM Header Generation

- It is no longer necessary to keep a physical `INIT.SDL` file in the `oem_sdl/` folder for every build.
- A new helper script, `create_constants.py`, parses an original OEM `INIT.SDL` once, extracts static data (headers, dictionaries), and stores them in the Python module `src/sdal_builder/init_constants.py`.
- `src/sdal_builder/main.py` now imports these OEM constants directly from `init_constants.py`, which makes the build process more self-contained and less dependent on external binary files at build time.

### 2. File Structure: Density and KD-Tree Outputs

- Density (traffic) layers are now written as **two** SDAL files per region, mirroring the layout of OEM discs:
  - `DENSxx0.SDL` — metadata and headers.
  - `DENSxx1.SDL` — raw tiled density payload.
- This split layout is important for compatibility with older Mazda/Saab head units that expect density overlays in this form.
- `KDTREE.SDL` generation has been fixed so that it now includes a correct `IDxPclHdr_t` header in front of the KD-tree data. This allows the navigation software to correctly interpret the global/world bounding box and the extent of spatial coverage.

### 3. Validation: Deep ISO Validator

- `validate_sdal_iso.py` has been extended into a **deep structural validator** instead of only checking file presence.
- New checks include:
  - **Parcel Chain Check:** Walks through each SDAL file byte by byte, verifying the parcel chain (`PclHdr_t` → length → padding → next header).
  - **OEM Signatures:** Verifies OEM-specific text signatures in `INIT.SDL`.
  - **Topology Linkage:** Confirms that `DB_ID` values in the global index (`CARTOTOP.SDL`) match IDs used inside cartographic files.
  - **B-Tree & K-Tree Sanity:** Checks for the presence and basic structure of search trees (B-tree and KD-tree parcels) used for spatial and ID-based lookup.
- This validator is now the recommended final step before burning a disc or deploying an image to a test vehicle.

### 4. Encoding and Compression Defaults

- For maximum compatibility and easier debugging, compression for text-heavy data (such as `POINAMES`, `NAV` parcels, etc.) is currently forced to **`NO_COMPRESSION`**.
- All SDAL headers and flags are still populated correctly, so compression (Huffman/SZIP) can be re-enabled later without changing the external file layout.
- This behaviour is temporary and aimed at ensuring that structural issues are solved first before re-introducing complex compression paths.

### 5. New Top-Level Helper Scripts

- **`create_constants.py`** — one-time setup tool that converts an OEM donor `INIT.SDL` into a code-based constants module (`init_constants.py`).
- **`validate_sdal_iso.py` (updated)** — the main QA/validation entry point for checking SDAL ISO images before burning. It now combines simple presence checks with deep structural validation (see above).
